...
use crate::{
    ...
    domain::{
        agent_session::{
            AgentProcessId, AgentSession, AgentSessionId, AgentSessionState, ConfiguredAgentKey,
            LaunchToken,
        },
        config::{ConfigError, ProcessSpec, WorkspaceConfig},
        notification::{Notification, NotificationId, NotificationScope},
        port::{
            AgentSessionStore, ConfigWatcher, EditorError, Notifier, PathCompleter, ProcessHandle,
            ProcessMetrics, ProcessRunner, ProjectRegistry, SettingsStore,
        },
        process::{
...
/// How often per-process resource usage is resampled for the sidebar.
...
/// Shown when a jump-to-attention finds nothing awaiting input.
const NO_ATTENTION_WAITING: &str = "nothing is waiting on you";
/// Prefix for a failure to launch the editor.
...
const AGENT_NAME_TAKEN: &str = "an agent with that name already exists";
/// Shown when a start is refused because a live provider still owns the session
/// (another Muster instance, or this one's previous provider after a change).
const AGENT_SESSION_OWNED_ELSEWHERE: &str = "this agent's session is already running";
...
/// Prefix for the numbered fallback name of an autogenerated configured agent.
...
/// Why a pane is being launched, which selects the resume semantics for a linked
/// agent session. Automatic launches (startup autostart, a policy-driven restart)
/// use [`AgentSession::restore_command`], so an owned-but-uncaptured reporter
/// session is surfaced as stopped rather than silently relaunched as a fresh
/// conversation; an explicit user launch uses [`AgentSession::reopen_command`],
/// starting a fresh conversation when there is nothing to resume.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum LaunchIntent {
    /// Muster initiated the launch (startup autostart or an automatic restart).
    Automatic,
    /// The user explicitly asked for the launch (start or restart action).
    UserInitiated,
}
...
pub struct App {
    ...
    /// A project directory queued to open in the editor. The runtime owns the
    /// terminal, so it performs the suspend/launch/restore off this field.
    pending_editor: Option<PathBuf>,
    /// Samples per-process resource usage, injected by the composition root.
    process_metrics: Option<Box<dyn ProcessMetrics>>,
    /// Latest resource usage per running pane, refreshed on the metrics tick.
    usage: HashMap<PaneId, sidebar::PaneUsage>,
    /// When usage is next resampled.
    metrics_deadline: Instant,
    /// A transient failure message shown as an error toast, cleared on the next
    /// key or when [`Self::expire_notice`] passes its deadline.
    notice: Option<String>,
    /// When the current notice auto-dismisses. Set lazily the first time the
    /// runtime loop observes a notice, cleared when the notice goes away.
    ...
    /// A transient, auto-dismissing feedback box shown over the pane area.
    toast: Option<Toast>,
    /// True between the left-press that dismissed a toast and its release, so the
    /// matching drag and up events are swallowed instead of reaching a
    /// mouse-reporting child as an orphaned release.
    toast_dismiss_capture: bool,
}
...
impl App {
    ...
    pub fn new(
        ...
        Self {
            ...
            usage: HashMap::new(),
            metrics_deadline: Instant::now() + METRICS_SAMPLE_INTERVAL,
            notice: None,
    ...
    pub fn set_settings_store(&mut self, store: Box<dyn SettingsStore>) {
        self.settings = match store.load() {
            ...
            Err(error) => {
                self.set_notice(format!("{SETTINGS_LOAD_ERROR}: {error}"));
    ...
    /// Takes the project directory queued to open in the editor, if any.
    pub fn take_pending_editor(&mut self) -> Option<PathBuf> {
        self.pending_editor.take()
    }

    /// Reports the outcome of an editor launch: a confirmation toast on success,
    /// an error notice otherwise.
    pub fn report_editor_result(&mut self, result: Result<(), EditorError>) {
        match result {
            Ok(()) => self.show_toast(OPENED_IN_EDITOR, ToastTone::Success),
            Err(error) => self.set_notice(format!("{EDITOR_OPEN_FAILED}: {error}")),
        }
    }

    /// Wires the per-process resource-usage sampler shown in the sidebar.
    pub fn set_process_metrics(&mut self, metrics: Box<dyn ProcessMetrics>) {
        self.process_metrics = Some(metrics);
    }
    ...
    fn toggle_desktop_notifications(&mut self) {
        // A saved toggle is a success, shown as a success toast; only the failure
        // paths become red notices.
        let outcome: Result<&str, String> = match &mut self.settings {
            SettingsState::Loaded { settings, store } => {
                // Reload from disk first: an external edit to another field
                // (editor.command, editor.terminal, ...) must not be reverted by
                // writing back the stale startup snapshot for the sake of one flag.
                // A load failure now means the file turned malformed since startup;
                // surface it rather than overwrite it blindly.
                match store.load() {
                    Ok(current) => {
                        let enabled = !*current.desktop_notifications();
                        let updated = current.with_desktop_notifications(enabled);
                        if store.save(&updated).is_ok() {
                            *settings = updated;
                            ...
                            Err(SETTINGS_SAVE_ERROR.to_string())
                        }
                    },
                    Err(error) => Err(format!("{SETTINGS_LOAD_ERROR}: {error}")),
                }
            },
            SettingsState::LoadFailed(error) => Err(format!("{SETTINGS_LOAD_ERROR}: {error}")),
            SettingsState::Unloaded => Err(SETTINGS_UNAVAILABLE.to_string()),
        };
        match outcome {
            Ok(message) => self.show_toast(message, ToastTone::Success),
    ...
    pub fn start(&mut self) {
        ...
        // rebuild it as a duplicate runtime row.
        self.link_configured_agent_sessions();
        for pane in self.autostart_panes() {
            // Resolve through `command_of` with automatic (restore) semantics: it
            // resumes a captured agent, surfaces an owned-but-uncaptured reporter
            // session as stopped rather than silently starting a fresh conversation,
            // and refuses an unlinked configured agent. `spawn` then refuses any
            // session a live owner (another Muster instance) still holds.
    ...
    fn autostart_panes(&self) -> Vec<PaneId> {
        self.workspace
            .processes()
            .iter()
...
mod tests {
    ...
    /// A metrics sampler that records whether it was asked to sample.
    struct FakeMetrics {
        called: std::rc::Rc<std::cell::RefCell<bool>>,
    }

    impl ProcessMetrics for FakeMetrics {
        fn sample(&mut self, _pids: &[u32]) -> HashMap<u32, crate::domain::port::ProcessSample> {
    ...
    impl AgentSessionStore for FakeAgentSessionStore {
        ...
        fn link_configured(&self, candidate: &AgentSession) -> Result<AgentSessionId, ConfigError> {
            ...
            if let Some(session) = sessions.iter_mut().find(|session| {
                ...
                *session = session
                    .clone()
                    .with_configured_command(*candidate.tool(), candidate.launch_command().clone());
                Ok(session.id().clone())
            } else {
    ...
    /// Showing a toast records it with a future auto-dismiss deadline.
    #[test]
    fn showing_a_toast_sets_a_future_dismiss_deadline() {
        let mut app = app_with(RestartPolicy::Never);
        let before = Instant::now();

        app.show_toast("copied", ToastTone::Success);

        assert!(app.next_toast_deadline().expect("deadline") > before);
        assert_eq!(
    ...
    /// A toast survives until its deadline, then a later tick clears it.
    #[test]
    fn a_toast_survives_until_its_deadline_then_clears() {
        let mut app = app_with(RestartPolicy::Never);
        app.show_toast("copied", ToastTone::Success);
        let deadline = app.next_toast_deadline().expect("deadline");

        assert!(!app.expire_toast(deadline - Duration::from_millis(1)));
        assert!(app.toast.is_some());
        assert!(app.expire_toast(deadline));
        assert!(app.toast.is_none());
        assert!(app.next_toast_deadline().is_none());
    }

    /// A drag copy raises a copied-to-clipboard toast.
    #[test]
    fn a_drag_copy_raises_a_copied_toast() {
        let mut app = app_with(RestartPolicy::Never);
        feed_pane(&mut app, b"hello world");
        let (_, main, _) = areas(app.frame_area);
        let (x, y) = (main.x + BORDER_THICKNESS, main.y + BORDER_THICKNESS);
    ...
    /// A double-click word copy raises a copied-to-clipboard toast.
    #[test]
    fn a_word_copy_raises_a_copied_toast() {
        let mut app = app_with(RestartPolicy::Never);
        feed_pane(&mut app, b"hello world");
        let (_, main, _) = areas(app.frame_area);
        let (x, y) = (main.x + BORDER_THICKNESS, main.y + BORDER_THICKNESS);
    ...
    /// A shown toast is drawn into the rendered frame.
    #[test]
    fn a_shown_toast_is_drawn_over_the_workspace() {
        use ratatui::{Terminal, backend::TestBackend};

        let mut app = app_with(RestartPolicy::Never);
        app.show_toast(TOAST_COPIED, ToastTone::Success);
        ...
        let rendered: String = terminal
            ...
            .collect();
        assert!(rendered.contains(TOAST_COPIED));
    }

    /// An error notice arms its deadline on assignment, then auto-dismisses
    /// without needing a keypress.
    #[test]
    fn an_error_notice_auto_dismisses_after_its_deadline() {
        let mut app = app_with(RestartPolicy::Never);
        app.set_notice("boom".to_string());

        let deadline = app.next_notice_deadline().expect("armed on assignment");
        assert!(!app.expire_notice(deadline - Duration::from_millis(1)));
        assert!(app.notice.is_some());
        assert!(app.expire_notice(deadline));
        assert!(app.notice.is_none());
        assert!(app.next_notice_deadline().is_none());
    }

    /// Replacing a notice resets its deadline rather than inheriting the old one.
    #[test]
    fn replacing_a_notice_resets_its_deadline() {
        let mut app = app_with(RestartPolicy::Never);
        app.set_notice("first".to_string());
        let first = app.next_notice_deadline().expect("armed");
        app.set_notice("second".to_string());
        let second = app.next_notice_deadline().expect("re-armed");

        assert!(second >= first);
        assert_eq!(app.notice.as_deref(), Some("second"));
    }

    /// Clicking inside a toast box closes it and consumes the click.
    #[test]
    fn clicking_a_toast_closes_it() {
        let mut app = app_with(RestartPolicy::Never);
        app.show_toast(TOAST_COPIED, ToastTone::Success);
        let (_, main, _) = areas(app.frame_area);
        let rect = toast::region(main, TOAST_COPIED, ToastTone::Success, 0).expect("toast region");
    ...
    /// Dismissing a toast captures the whole gesture: the press closes the toast
    /// and arms capture, the matching release clears it, so no orphaned release
    /// reaches a mouse-reporting child.
    #[test]
    fn dismissing_a_toast_captures_the_whole_gesture() {
        let mut app = app_with(RestartPolicy::Never);
        app.show_toast(TOAST_COPIED, ToastTone::Success);
        let (_, main, _) = areas(app.frame_area);
        let rect = toast::region(main, TOAST_COPIED, ToastTone::Success, 0).expect("toast region");
        ...
        assert!(app.toast.is_none());
        assert!(app.toast_dismiss_capture);
    ...
    /// Jump-to-attention moves the selection to the next process awaiting input,
    /// and reports when nothing is waiting.
    #[test]
    fn jump_to_attention_selects_the_next_waiting_process() {
        let (sender, _receiver) = bounded(16);
        let terminal = |id: u64, name: &str| {
            Process::builder()
                .id(PaneId::new(id))
        ...
        let workspace = Workspace::builder()
            .processes(vec![terminal(0, "a"), terminal(1, "b")])
            .build();
        let mut app = App::new(
            workspace,
            Box::new(FakeRunner),
        ...
        app.workspace
            .set_activity(PaneId::new(1), ActivityState::AwaitingInput);

        assert!(app.jump_to_attention());
        assert_eq!(*app.workspace.selected_index(), 1);

        app.workspace
            .set_activity(PaneId::new(1), ActivityState::Idle);
        assert!(!app.jump_to_attention());
        assert!(app.notice.is_some());
    }

    /// Jump-to-attention on an empty workspace still reports that nothing waits,
    /// rather than silently doing nothing.
    #[test]
    fn jump_to_attention_on_an_empty_workspace_reports_nothing() {
        let (sender, _receiver) = bounded(16);
        let mut app = App::new(
    ...
    /// `e` queues the active project's directory for the runtime to open.
    #[test]
    fn open_in_editor_queues_the_active_project_directory() {
        let mut app = app_with(RestartPolicy::Never);

        app.open_selected_in_editor();

        assert!(app.take_pending_editor().is_some());
        assert!(app.take_pending_editor().is_none());
    }

    /// A successful launch confirms with a toast; a failure surfaces a notice.
    #[test]
    fn editor_results_are_reported() {
        let mut app = app_with(RestartPolicy::Never);

        app.report_editor_result(Ok(()));
        assert_eq!(
            app.toast.as_ref().map(|toast| toast.message().as_str()),
            Some(OPENED_IN_EDITOR)
        );

        app.report_editor_result(Err(crate::domain::port::EditorError::NoEditor));
        assert!(
            app.notice
                .as_deref()
                .is_some_and(|notice| notice.contains(EDITOR_OPEN_FAILED))
        );
    }

    /// Usage is resampled only once the metrics deadline has passed.
    #[test]
    fn metrics_are_sampled_only_after_the_deadline() {
        let mut app = app_with(RestartPolicy::Never);
        // Seed a cached usage entry so a tick is not short-circuited as having
        // nothing to sample, letting this test observe the deadline behavior.
        app.usage.insert(
            PaneId::new(PANE),
        ...
        app.set_process_metrics(Box::new(FakeMetrics {
            called: called.clone(),
        }));
        let deadline = app.next_metrics_deadline().expect("metrics wired");

        assert!(!app.sample_metrics(deadline - Duration::from_millis(1)));
        assert!(!*called.borrow());
        app.sample_metrics(deadline);
        assert!(
            *called.borrow(),
            "sampling happens once the deadline passes"
        );
    }

    /// With no live process and no cached usage there is nothing to sample, so the
    /// metrics timer is disarmed and the runtime loop sleeps instead of waking every
    /// interval to no-op on an empty or fully stopped workspace.
    #[test]
    fn metrics_timer_is_disarmed_when_nothing_is_live() {
        let (sender, _receiver) = bounded(16);
        let mut app = App::new(
            Workspace::builder().processes(Vec::new()).build(),
            Box::new(FakeRunner),
        ...
        app.set_process_metrics(Box::new(FakeMetrics {
            called: std::rc::Rc::new(std::cell::RefCell::new(false)),
        }));

        assert!(
            app.next_metrics_deadline().is_none(),
            "an idle workspace with nothing to sample disarms the metrics timer"
        );
    }

    /// A configured agent with no session is linked to a fresh durable session,
    /// so the shared spawn path resumes and captures it like a runtime session.
    #[test]
    fn a_configured_agent_is_linked_to_a_fresh_session() {
        let (sender, _receiver) = bounded(16);
        let agent = ProcessSpec::builder()
            .name(ProcessName::try_new("claude").unwrap())
            .command(Some(CommandLine::try_new("claude").unwrap()))
            .build()
            .to_process(PaneId::new(0), ProcessKind::Agent);
        let mut app = App::new(
            Workspace::builder().processes(vec![agent]).build(),
            Box::new(FakeRunner),
            sender,
            Rect::new(0, 0, 80, 24),
            Box::new(FakeCompleter),
        ...
        app.link_configured_agent_sessions();

        let id = app.workspace.processes()[0]
            .agent_session_id()
            .clone()
            .expect("linked to a session");
        let sessions = recorder.sessions.borrow();
        assert_eq!(sessions.len(), 1);
        assert_eq!(sessions[0].id(), &id);
        assert_eq!(sessions[0].name().as_ref(), "claude");
    }

    /// Re-linking a configured agent reuses its existing session by name and
    /// project rather than orphaning the conversation with a new one.
    #[test]
    fn a_configured_agent_reuses_its_existing_session() {
        let (sender, _receiver) = bounded(16);
        let agent = ProcessSpec::builder()
            .name(ProcessName::try_new("claude").unwrap())
            .command(Some(CommandLine::try_new("claude").unwrap()))
        ...
        let recorder = SessionRecorder::default();
        let existing_id = AgentSessionId::generate().unwrap();
        recorder.sessions.borrow_mut().push(
            AgentSession::builder()
                ...
                .configured_key(Some(
                    ConfiguredAgentKey::of(&ProcessName::try_new("claude").unwrap()).unwrap(),
                ))
        ...
        assert_eq!(
            recorder.sessions.borrow().len(),
            1,
            "no new session created"
        );
    }

    /// A previously launched reporter-based configured agent (Codex) with no
    /// captured native id must not be autostarted into a fresh conversation:
    /// automatic launches restore (yielding no command, so it stays stopped),
    /// while an explicit user start reopens with a fresh session command.
    #[test]
    fn autostart_does_not_reopen_an_uncaptured_reporter_session() {
        let (sender, _receiver) = bounded(16);
        let agent = ProcessSpec::builder()
            .name(ProcessName::try_new("codex").unwrap())
        ...
        recorder.sessions.borrow_mut().push(
            AgentSession::builder()
                ...
                .configured_key(Some(
                    ConfiguredAgentKey::of(&ProcessName::try_new("codex").unwrap()).unwrap(),
                ))
                // Launched (owner bound) but never captured a native id.
                .owner_process_id(Some(AgentProcessId::try_new(4242).unwrap()))
                .state(AgentSessionState::Closed)
                .build(),
        );
        app.set_agent_session_store(Box::new(FakeAgentSessionStore { recorder }));

        app.link_configured_agent_sessions();
        let pane = PaneId::new(0);

        assert!(
            matches!(app.command_of(pane, LaunchIntent::Automatic), Ok(None)),
            "autostart must not silently reopen an uncaptured reporter session"
        );
        assert!(
            matches!(
                app.command_of(pane, LaunchIntent::UserInitiated),
                Ok(Some(_))
            ),
            "an explicit start reopens with a fresh session command"
        );
    }

    /// Distinctly named configured agents link to distinct sessions keyed by
    /// their names, so their conversations and ownership claims never collide.
    /// (Two agents sharing a name is rejected at config load, so it never
    /// reaches the linker.)
    #[test]
    fn distinct_configured_agents_get_distinct_sessions() {
        let (sender, _receiver) = bounded(16);
        let agent = |id, name| {
            ProcessSpec::builder()
                .name(ProcessName::try_new(name).unwrap())
                .command(Some(CommandLine::try_new("claude").unwrap()))
                .build()
                .to_process(PaneId::new(id), ProcessKind::Agent)
        };
        let mut app = App::new(
            Workspace::builder()
                .processes(vec![agent(0, "claude"), agent(1, "opus")])
                .build(),
            Box::new(FakeRunner),
            sender,
            Rect::new(0, 0, 80, 24),
        ...
        let first = app.workspace.processes()[0].agent_session_id().clone();
        let second = app.workspace.processes()[1].agent_session_id().clone();
        assert!(first.is_some() && second.is_some());
        assert_ne!(first, second, "distinct agents must not share a session");
        assert_eq!(recorder.sessions.borrow().len(), 2);
    }

    /// Retiring a pane drops its metrics sample, so a later process that reuses
    /// the pane id does not briefly show the retired process's pid, memory, or cpu
    /// until the next metrics tick.
    #[test]
    fn retiring_a_pane_drops_its_usage_sample() {
        let mut app = app_with(RestartPolicy::Never);
        ...
        assert!(
            !app.usage.contains_key(&pane),
            "a retired pane's stale sample must not linger for a reused id"
        );
    }

    /// Builds an app whose sole configured agent `claude` is linked to a durable
    /// session already held by a live owner (this test process), so the live-owner
    /// gate applies. Returns the app and that session's id. The agent is pane 0.
    #[cfg(unix)]
    fn app_with_live_owned_configured_agent() -> (App, AgentSessionId) {
        let (sender, _receiver) = bounded(16);
        let agent = ProcessSpec::builder()
            .name(ProcessName::try_new("claude").unwrap())
            .command(Some(CommandLine::try_new("claude").unwrap()))
    ...
    /// A configured agent whose session is held by a live owner (another Muster
    /// instance sharing the project) is detected as owned elsewhere, so no launch
    /// path spawns it into a lost ownership claim.
    #[cfg(unix)]
    #[test]
    fn a_configured_agent_owned_by_a_live_instance_is_not_launchable() {
        let (mut app, existing_id) = app_with_live_owned_configured_agent();

        assert_eq!(
            app.workspace.processes()[0].agent_session_id().as_ref(),
            Some(&existing_id),
            "the configured agent links to the live-owned session"
        );
        assert!(
            app.agent_session_owned_by_live_process(PaneId::new(0)),
            "a live owner must gate the launch so the ownership claim is not lost"
        );

        // Reaching `spawn` directly (as the relink-then-start and add-then-reconcile
        // paths do) must still refuse, not launch a competing provider.
        app.spawn(
            PaneId::new(0),
            Some(CommandLine::try_new("claude").unwrap()),
            None,
        );
        assert!(
            !app.panes.contains_key(&PaneId::new(0)),
            "spawn must not launch a provider for a live-owned session"
        );
        assert_eq!(app.notice.as_deref(), Some(AGENT_SESSION_OWNED_ELSEWHERE));
    }

    /// A restart that the live-owner gate blocks settles the pane out of
    /// `Restarting` rather than hanging there forever with no pending retry, so a
    /// manual start can relaunch it once the other owner exits.
    #[cfg(unix)]
    #[test]
    fn a_gated_restart_settles_out_of_restarting() {
        let (mut app, _id) = app_with_live_owned_configured_agent();
        let pane = PaneId::new(0);
        app.workspace.set_state(pane, ProcessState::Restarting);

        app.spawn(pane, Some(CommandLine::try_new("claude").unwrap()), None);

        assert_eq!(
            *app.workspace.process(pane).unwrap().state(),
            ProcessState::Exited,
            "a gated restart must not stay stuck in Restarting"
        );
        assert!(!app.panes.contains_key(&pane), "no provider was launched");
    }

    /// Changing a running configured agent's provider retains the live-owner gate,
    /// so the new provider cannot start and share the durable session while the
    /// old one is still running.
    #[cfg(unix)]
    #[test]
    fn changing_a_configured_provider_keeps_the_live_owner_gate() {
        let (sender, _receiver) = bounded(16);
        // The row is renamed-in-place to a codex command; its name (the key) is
        // unchanged, so it links to the existing claude-owned session.
        let agent = ProcessSpec::builder()
            .name(ProcessName::try_new("claude").unwrap())
            .command(Some(CommandLine::try_new("codex").unwrap()))
            .build()
        ...
        recorder.sessions.borrow_mut().push(
            AgentSession::builder()
                ...
                .project(PathBuf::from("/here/muster.yml"))
                .launch_command(CommandLine::try_new("claude").unwrap())
                .native_id(Some(NativeSessionId::try_new("claude-thread").unwrap()))
                .configured_key(Some(
                    ConfiguredAgentKey::of(&ProcessName::try_new("claude").unwrap()).unwrap(),
                ))
        ...
        let session = sessions
            ...
            .unwrap();
        assert_eq!(*session.tool(), AgentTool::Codex, "the provider is updated");
        assert!(
            session.native_id().is_none(),
            "the old provider's conversation cannot be resumed by the new one"
        );
        drop(sessions);
        assert!(
            app.agent_session_owned_by_live_process(PaneId::new(0)),
            "the retained live owner still gates the new provider's launch"
        );
    }

    /// A leftover configured record (its muster.yml row removed) is not restored
    /// as a runtime session, so a removed agent is not silently relaunched.
    #[test]
    fn a_removed_configured_agent_is_not_restored_as_a_session() {
        let (sender, _receiver) = bounded(16);
        let mut app = App::new(
            Workspace::builder().processes(vec![]).build(),
            Box::new(FakeRunner),
        ...
        recorder.sessions.borrow_mut().push(
            AgentSession::builder()
                ...
                .native_id(Some(NativeSessionId::try_new("abc").unwrap()))
                .state(AgentSessionState::Open)
                .build(),
        );
        app.set_agent_session_store(Box::new(FakeAgentSessionStore { recorder }));

        app.restore_open_agent_sessions();

        assert!(
            app.workspace.processes().is_empty(),
            "a configured record must not be restored as a runtime session"
        );
    }

    /// Editing a configured agent's command refreshes its session's stored launch
    /// command while keeping the same durable session, so it never relaunches the
    /// stale command.
    #[test]
    fn a_configured_command_edit_refreshes_the_session() {
        let (sender, _receiver) = bounded(16);
        let agent = ProcessSpec::builder()
            .name(ProcessName::try_new("claude").unwrap())
            .command(Some(CommandLine::try_new("claude --model opus").unwrap()))
            .build()
            .to_process(PaneId::new(0), ProcessKind::Agent);
        let mut app = App::new(
            Workspace::builder().processes(vec![agent]).build(),
        ...
        let recorder = SessionRecorder::default();
        let id = AgentSessionId::generate().unwrap();
        recorder.sessions.borrow_mut().push(
            AgentSession::builder()
                .id(id.clone())
                .name(ProcessName::try_new("claude").unwrap())
        ...
        assert_eq!(
            app.workspace.processes()[0].agent_session_id().as_ref(),
            Some(&id),
            "reuses the same durable session"
        );
        let sessions = recorder.sessions.borrow();
        let session = sessions.iter().find(|session| session.id() == &id).unwrap();
        assert_eq!(session.launch_command().as_ref(), "claude --model opus");
    }

    /// When a configured agent's session cannot be persisted, it is left unlinked
    /// and `command_of` refuses to launch it, so no path (autostart or a manual
    /// start) spawns it into an untracked conversation.
    #[test]
    fn a_configured_agent_that_cannot_persist_is_not_launchable() {
        let (sender, _receiver) = bounded(16);
        let agent = ProcessSpec::builder()
            .name(ProcessName::try_new("claude").unwrap())
            .command(Some(CommandLine::try_new("claude").unwrap()))
        ...
        assert!(
            app.workspace.processes()[0].agent_session_id().is_none(),
            "an unpersisted session must not bind the pane"
        );
        assert!(
            matches!(
                app.command_of(PaneId::new(0), LaunchIntent::Automatic),
                Ok(None)
            ),
            "an unlinked configured agent must not launch its raw command"
        );
    }
    ...
    /// Test settings with the given notification toggle and an editor that
    /// defaults to the environment (both fields empty).
    fn test_settings(desktop_notifications: bool) -> Settings {
        settings_with_editor(desktop_notifications, "")
    }

    fn settings_with_editor(desktop_notifications: bool, editor_command: &str) -> Settings {
        Settings::builder()
            .desktop_notifications(desktop_notifications)
            .editor(
                crate::domain::settings::EditorSettings::builder()
    ...
    /// A store backed by an in-memory "disk" copy that `load` returns and `save`
    /// replaces, so a test can mutate the on-disk settings between load and toggle
    /// to model an external edit while muster runs.
    #[derive(Clone)]
    struct RecordingSettingsStore {
        on_disk: Rc<RefCell<Settings>>,
    ...
    impl RecordingSettingsStore {
        fn new(initial: Settings) -> Self {
            Self {
                on_disk: Rc::new(RefCell::new(initial)),
    ...
    impl SettingsStore for RecordingSettingsStore {
        fn load(&self) -> Result<Settings, ConfigError> {
            Ok(self.on_disk.borrow().clone())
        }

        fn save(&self, settings: &Settings) -> Result<(), ConfigError> {
            *self.on_disk.borrow_mut() = settings.clone();
    ...
    #[test]
    fn toggling_notifications_preserves_an_externally_edited_field() {
        let mut app = app_with(RestartPolicy::Never);
        let store = RecordingSettingsStore::new(settings_with_editor(true, "old-editor"));
        app.set_settings_store(Box::new(store.clone()));
        // An external process rewrites editor.command while muster is running.
        *store.on_disk.borrow_mut() = settings_with_editor(true, "new-editor");
        ...
        let saved = store
            ...
            .borrow()
            .clone()
            .expect("the toggle must persist the setting");
        assert_eq!(
            saved.editor().command().as_str(),
            "new-editor",
            "the externally edited field survives the notification toggle"
        );
        assert!(
            !*saved.desktop_notifications(),
            "the notification flag is still flipped and saved"
        );
    }
    ...
    /// configured process (origin Configured), restoring autostart control the
    /// disposable-session flow does not offer.
    #[test]
    fn adding_an_agent_creates_a_configured_process() {
        let (mut app, _recorder) = flow_app(vec![], empty_workspace_config(), "/here/muster.yml");

        app.add_configured_process(ProcessKind::Agent, &[
            "claude".to_string(),
            "claude".to_string(),
        ]);
        ...
        let added = app
            ...
            .processes()
            .iter()
            .find(|process| process.name().as_ref() == "claude")
            .expect("the configured agent was added");
        assert_eq!(*added.kind(), ProcessKind::Agent);
        assert_eq!(*added.origin(), ProcessOrigin::Configured);
    }

    /// `A` opens the disposable agent-session picker, the flow the add form used
    /// to occupy.
    #[test]
    fn shift_a_opens_the_agent_session_picker() {
        let (mut app, _recorder) = flow_app(vec![], empty_workspace_config(), "/here/muster.yml");
    ...
    /// A composed launch command is supported without an explicit resume template:
    /// resume arguments are prepended after the executable, so the session is
    /// created and launched rather than rejected.
    #[test]
    fn a_compound_agent_command_is_resumable_and_persisted() {
        let (mut app, sessions, spawns) = agent_app(Vec::new());
        ...
        assert_eq!(
            sessions.sessions.borrow().len(),
            1,
            "the session is persisted"
        );
        assert_eq!(spawns.requests.borrow().len(), 1, "and launched");
    }
